...
def save_results_csv(results_data: Dict, filename: str = "benchmark_results.csv") -> None:
    """Enhanced CSV export with baseline/speedup support"""
    Path(filename).parent.mkdir(parents=True, exist_ok=True)
    
    with open(filename, 'w', newline='') as csvfile:
        # Handle the new nested structure from benchmark.py
        if 'optimized' in results_data:
            results_dict = results_data['optimized']
        ...
        # Add baseline and speedup columns if available
        if baseline_dict:
            fieldnames.extend(['baseline_median_ms', 'speedup_median', 'improvement_pct'])
        ...
        for config, stats in results_dict.items():
            ...
            # Add baseline/speedup data if available
            if config in baseline_dict:
                row['baseline_median_ms'] = baseline_dict[config]['median_ms']
            if config in speedup_dict:
                row.update({
                    'speedup_median': speedup_dict[config]['speedup_median'],
                    'improvement_pct': speedup_dict[config]['improvement_pct']
                })
...
def save_results_json(results_data: Dict, filename: str = "benchmark_results.json") -> None:
    """Enhanced JSON export with baseline/speedup support"""
    Path(filename).parent.mkdir(parents=True, exist_ok=True)
    ...
    if 'optimized' in results_data:
        output.update(results_data)  # New format with baseline/speedup
    else:
        output['results'] = results_data  # Old format
...
def format_performance_report(results_data: Dict) -> str:
    """Enhanced report with baseline/speedup metrics"""
    # Handle both old and new result formats
    if 'optimized' in results_data:
        results_dict = results_data['optimized']
        speedup_dict = results_data.get('speedup', {})
    ...
    # Add speedup summary if baseline data exists
    if speedup_dict:
        speedups = [s['speedup_median'] for s in speedup_dict.values()]
    ...
    for config, stats in results_dict.items():
        ...
        if config in speedup_dict:
            speedup_info = speedup_dict[config]
            report.append(f"  🚀 Speedup: {speedup_info['speedup_median']:.1f}x ({speedup_info['improvement_pct']:.1f}% improvement)")
